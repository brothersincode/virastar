  ...
  Virastar.prototype = {
    ...
    cleanup: function cleanup (text, options) {
      ...
      if (opts.preserve_URIs) {
        ...
        text = text.replace(newRegExp(pattern), function (matched) {
          uris.push(matched);
      ...
      if (opts.fix_persian_glyphs) {
        text = that.fixPersianGlyphs(text, opts);
      }
      ...
      if (opts.fix_hamzeh) {
        if (opts.fix_hamzeh_arabic) {
          text = that.fixHamzehArabic(text, opts);
        }

        text = that.fixHamzeh(text, opts);
      } else if (opts.fix_suffix_spacing) {
        if (opts.fix_hamzeh_arabic) {
          text = that.fixHamzehArabicAlt(text, opts);
        }
      ...
      if (opts.fix_spacing_for_braces_and_quotes) {
        text = that.fixBracesSpacing(text, opts);
      }
      ...
      if (opts.fix_spacing_for_punctuations) {
        text = that.fixPunctuationSpacing(text, opts);
      }
      ...
      if (opts.markdown_normalize_braces) {
        text = that.markdownNormalizeBraces(text, opts);
      }

      if (opts.markdown_normalize_lists) {
        text = that.markdownNormalizeLists(text, opts);
      }

      // doing it again after `fixPunctuationSpacing()`
      if (opts.fix_spacing_for_braces_and_quotes) {
        text = that.fixBracesSpacingInside(text, opts);
      }

      if (opts.fix_diacritics) {
        text = that.fixDiacritics(text, opts);
      }
      ...
      if (opts.cleanup_zwnj) {
        text = that.cleanupZWNJLate(text, opts);
      }
    ...
    cleanupZWNJLate: function cleanupZWNJLate (text, options) {
      return text
    ...
    fixHamzehArabic: function fixHamzehArabic (text, options) {
      return text
    ...
    fixHamzehArabicAlt: function fixHamzehArabicAlt (text, options) {
      return text
    ...
    convertPersianNumbers: function convertPersianNumbers (text, options) {
    ...
    fixNumeralSymbols: function fixNumeralSymbols (text, options) {
      return text
        ...
        // props @ebraminio/persiantools
        .replace(newRegExp('([' + this.digits + ']) ?%'), '$1٪')
        // replaces dots between numbers into decimal separator (U+066B)
        // props @ebraminio/persiantools
        .replace(newRegExp('([' + this.digits + '])\\.(?=[' + this.digits + '])'), '$1٫')
    ...
    fixPerfixSpacing: function fixPerfixSpacing (text, options) {
      return text
        .replace(/((\s|^)ن?می) ([^ ])/g, '$1\u200c$3')
        .replace(/((\s|^)بی) ([^ ])/g, '$1\u200c$3') // props @zoghal
      ;
    },
    ...
    fixPunctuationSpacing: function fixPunctuationSpacing (text, options) {
      return text
    ...
    fixBracesSpacing: function fixBracesSpacing (text, options) {
      return text
    ...
    fixBracesSpacingInside: function fixBracesSpacingInside (text, options) {
      return text
    ...
    markdownNormalizeBraces: function markdownNormalizeBraces (text, options) {
      return text
    ...
    markdownNormalizeLists: function markdownNormalizeLists (text, options) {
      return text
    ...
    // `\u064e\u0650\u064f\u064b\u064d\u064c\u0651\u06c0`
    fixDiacritics: function fixDiacritics (text, options) {
      return text
