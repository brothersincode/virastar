...
(function (name, global, definition) {
  ...
  var defaults = {
    ...
    // cleanup_rlm: true, // DEPRECATED
    cleanup_direction_marks: true,
    ...
    preserve_code_blocks: true, // Fenced Code Blocks
    preserve_inline_codes: true, // backticks
    preserve_HTML: true,
    preserve_nbsps: true,
    preserve_URIs: true,
  ...
  var entities = {
    ...
    'ZWNJ;': '\u200c',
    'zwnj;': '\u200c',
    'lrm;': '\u200e', // wrongly used as zwnj
    'rlm;': '\u200f', // wrongly used as zwnj
  ...
  function cleanup (text, options) {
    ...
    // single space padding around the string
    text = ' ' + text + ' ';
    ...
    if (opts.preserve_code_blocks) {
      var codeBlocks = [];
      text = text.replace(/(\n|\s|^)```[\S\s]*?```/g, function (matched) {
        codeBlocks.push(matched);
        return ' __CODEBLOCKS__PRESERVER__ ';
      });
    }

    // preserves inline codes in the text
    if (opts.preserve_inline_codes) {
      var inlineCodes = [];
      text = text.replace(/(\`.*?\`)/g, function (matched) {
        inlineCodes.push(matched);
        return ' __INLINECODES__PRESERVER__ ';
      });
    }
    ...
    // preserves hashtags in the text
    // @source https://stackoverflow.com/a/67751172
    if (opts.preserve_hashtags) {
      var hashtags = [];
      text = text.replace(/#[\p{L}\p{Nd}_]+/gmu, function (matched) {
        hashtags.push(matched);
        return ' __HASHTAG__PRESERVER__ ';
      });
    }
    ...
    if (opts.cleanup_direction_marks) {
      text = cleanupDirectionMarks(text);
    }
    ...
    // bringing back hashtags
    if (opts.preserve_hashtags && hashtags.length) {
      text = text.replace(/[ ]?__HASHTAG__PRESERVER__[ ]?/g, function () {
        return hashtags.shift();
      });
    }
    ...
    // bringing back inline codes
    if (opts.preserve_inline_codes) {
      text = text.replace(/[ ]?__INLINECODES__PRESERVER__[ ]?/g, function () {
        return inlineCodes.shift();
      });
    }

    // bringing back code blocks
    if (opts.preserve_code_blocks) {
      text = text.replace(/[ ]?__CODEBLOCKS__PRESERVER__[ ]?/g, function () {
  ...
  function cleanupZWNJ (text) {
    return text
      ...
      // cleans zwnj before and after numbers, english words, space, and punctuation
  ...
  function cleanupDirectionMarks (text) {
    return text
  ...
  function normalizeDates (text) {
    return text
      ...
      .replace(/([0-9۰-۹]{1,4})[ \t\u200c]*\/[ \t\u200c]*([0-9۰-۹]{1,2})[ \t\u200c]*\/[ \t\u200c]*([0-9۰-۹]{1,4})/gm, '$1/$2/$3')
  ...
  function cleanupKashidas (text) {
    return text
      // converts kashida between numbers to n-dash
  ...
  Virastar.prototype = {
    ...
    // cleanupDirectionMarks: cleanupDirectionMarks,
