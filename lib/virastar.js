  ...
  Virastar.prototype = {
    ...
    cleanup: function cleanup (text, options) {
      ...
      if (opts.decode_htmlentities) {
        text = that.decodeHTMLEntity(text, opts);
      }
    ...
    cleanupZWNJ: function cleanupZWNJ (text) {
    ...
    decodeHTMLEntity: function decodeHTMLEntity (text, options) {
      return text.replace(/&#(\d+);/g, function (matched, dec) {
