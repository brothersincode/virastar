/*!
* Virastar - v0.21.0-dev - 2020-03-28
...
(function (name, global, definition) {
  ...
  function cleanup (text, options) {
    ...
    if (opts.preserve_URIs) {
      ...
      text = text.replace(/(!)?\[(.*[^\]])?\]\((.*[^)])\)/g, function (matched, img, title, link) {
        // again checks for markdown image links within
        if (title) {
          title = title.replace(/!\[(.*?)\]\((.*?[^)])\)/g, function (image, alt, src) {
            mdlinks.push(src.trim());
            return '![' + (alt || '') + '](__MD_LINK__PRESERVER__)'; // no padding!
          });
        }

        mdlinks.push(link.trim());
        return (img || '') + '[' + (title || '') + '](__MD_LINK__PRESERVER__)'; // no padding!
      });
    ...
    if (opts.decode_htmlentities) {
      text = decodeHTMLEntities(text, opts);
    }
    ...
    if (opts.normalize_eol) {
      text = normalizeEOL(text, opts);
    }

    if (opts.fix_persian_glyphs) {
      text = fixPersianGlyphs(text, opts);
    }

    if (opts.fix_dashes) {
      text = fixDashes(text, opts);
    }

    if (opts.fix_three_dots) {
      text = fixThreeDots(text, opts);
    }

    if (opts.normalize_ellipsis) {
      text = normalizeEllipsis(text, opts);
    }

    if (opts.fix_english_quotes_pairs) {
      text = fixEnglishQuotesPairs(text, opts);
    }

    if (opts.fix_english_quotes) {
      text = fixEnglishQuotes(text, opts);
    }

    if (opts.fix_hamzeh) {
      if (opts.fix_hamzeh_arabic) {
        text = fixHamzehArabic(text, opts);
      }

      text = fixHamzeh(text, opts);
    } else if (opts.fix_suffix_spacing) {
      if (opts.fix_hamzeh_arabic) {
        text = fixHamzehArabicAlt(text, opts);
      }

      text = fixSuffixSpacingHamzeh(text, opts);
    }

    if (opts.cleanup_rlm) {
      text = cleanupRLM(text, opts);
    }

    if (opts.cleanup_zwnj) {
      text = cleanupZWNJ(text, opts);
    }

    if (opts.fix_arabic_numbers) {
      text = fixArabicNumbers(text, opts);
    }
    ...
    text = text.replace(/(^|\s+)([[({"'“«]?)(\S+)([\])}"'”»]?)(?=($|\s+))/g,
      function (matched, before, leadings, word, trailings, after) {
        ...
        if (opts.fix_english_numbers) {
          matched = fixEnglishNumbers(matched, opts);
        }

        if (opts.fix_numeral_symbols) {
          matched = fixNumeralSymbols(matched, opts);
        }

        if (opts.fix_punctuations) {
          matched = fixPunctuations(matched, opts);
        }

        if (opts.fix_misc_non_persian_chars) {
          matched = fixMiscNonPersianChars(matched, opts);
        }

        if (opts.fix_question_mark) {
          matched = fixQuestionMark(matched, opts);
        }
    ...
    if (opts.fix_perfix_spacing) {
      text = fixPerfixSpacing(text, opts);
    }

    if (opts.fix_suffix_spacing) {
      text = fixSuffixSpacing(text, opts);
    }

    if (opts.fix_suffix_misc) {
      text = fixSuffixMisc(text, opts);
    }

    if (opts.fix_spacing_for_braces_and_quotes) {
      text = fixBracesSpacing(text, opts);
    }

    if (opts.cleanup_extra_marks) {
      text = cleanupExtraMarks(text, opts);
    }

    if (opts.fix_spacing_for_punctuations) {
      text = fixPunctuationSpacing(text, opts);
    }

    if (opts.kashidas_as_parenthetic) {
      text = kashidasAsParenthetic(text, opts);
    }

    if (opts.cleanup_kashidas) {
      text = cleanupKashidas(text, opts);
    }

    if (opts.markdown_normalize_braces) {
      text = markdownNormalizeBraces(text, opts);
    }

    if (opts.markdown_normalize_lists) {
      text = markdownNormalizeLists(text, opts);
    }

    // doing it again after `fixPunctuationSpacing()`
    if (opts.fix_spacing_for_braces_and_quotes) {
      text = fixBracesSpacingInside(text, opts);
    }

    if (opts.fix_diacritics) {
      text = fixDiacritics(text, opts);
    }

    if (opts.cleanup_spacing) {
      text = cleanupSpacing(text, opts);
    }

    if (opts.cleanup_zwnj) {
      text = cleanupZWNJLate(text, opts);
    }

    if (opts.cleanup_line_breaks) {
      text = cleanupLineBreaks(text, opts);
    }
    ...
    if (opts.cleanup_begin_and_end) {
      text = cleanupBeginAndEnd(text, opts);
    } else {
  ...
  // props @ebraminio/persiantools
  function cleanupZWNJ (text, options) {
    return text
  ...
  function cleanupZWNJLate (text, options) {
    return text
  ...
  // @props: @substack/node-ent
  function decodeHTMLEntities (text, options) {
  ...
  function normalizeEOL (text, options) {
    return text
      .replace(/(\r?\n)|(\r\n?)/g, '\n')
    ;
  }

  function fixDashes (text, options) {
    return text
  ...
  function fixThreeDots (text, options) {
    return text
      .replace(/\s*\.{3,}/g, '…')
    ;
  }

  function normalizeEllipsis (text, options) {
    return text
  ...
  function fixEnglishQuotesPairs (text, options) {
    return text
      .replace(/(“)(.+?)(”)/g, '«$2»')
    ;
  }
  ...
  function fixEnglishQuotes (text, options) {
    return text
      .replace(/(["'`]+)(.+?)(\1)/g, '«$2»')
    ;
  }

  function fixHamzeh (text, options) {
    return text
  ...
  function fixHamzehArabic (text, options) {
    return text
  ...
  function fixHamzehArabicAlt (text, options) {
    return text
  ...
  function cleanupRLM (text, options) {
    return text
  ...
  function fixPersianGlyphs (text, options) {
    return arrReplace(text, glyphs);
  }

  // props @ebraminio/persiantools
  function fixMiscNonPersianChars (text, options) {
  ...
  function fixPunctuations (text, options) {
    return charReplace(text, ',;', '،؛');
  }

  // replaces question marks with its persian equivalent
  function fixQuestionMark (text, options) {
    return text
      .replace(/(\?)/g, '\u061F') // \u061F = ؟
    ;
  }
  ...
  function fixPerfixSpacing (text, options) {
    var replacement = '$1\u200c$3';
    return text
  ...
  function fixSuffixSpacing (text, options) {
    var replacement = '$1\u200c$2';
    return text
  ...
  function fixSuffixSpacingHamzeh (text, options) {
  ...
  function fixSuffixMisc (text, options) {
    return text
  ...
  function cleanupExtraMarks (text, options) {
    return text
  ...
  function kashidasAsParenthetic (text, options) {
    return text
      .replace(/(\s)\u0640+/g, '$1–')
      .replace(/\u0640+(\s)/g, '–$1')
    ;
  ...
  function cleanupKashidas (text, options) {
    return text
  ...
  function fixPunctuationSpacing (text, options) {
    return text
      // : ; , . ! ? and their persian equivalents should have
      // one space after and no space before
      .replace(/[ \t\u200c]*([:;,؛،.؟?!]{1})[ \t\u200c]*/g, '$1 ')
      ...
      // removes space between same marks (in case `cleanup_extra_marks` disabled)
      .replace(/!\s{1,}!/g, '!!')
      .replace(/\?\s{1,}\?/g, '??')
      .replace(/؟\s{1,}؟/g, '؟؟')
    ;
  ...
  function fixBracesSpacing (text, options) {
  ...
  function fixBracesSpacingInside (text, options) {
    var replacement = '$1$2$3';
    return text
  ...
  function markdownNormalizeBraces (text, options) {
    return text
      ...
      .replace(/\[\[\s*(.*?)\s*\]\]/g, '[[$1]]')
      .replace(/\{\{\s*(.*?)\s*\}\}/g, '{{$1}}')
      .replace(/\{\{\{\s*(.*?)\s*\}\}\}/g, '{{{$1}}}') // mustache escape
  ...
  function markdownNormalizeLists (text, options) {
    return text
  ...
  function cleanupLineBreaks (text, options) {
    return text.replace(/(\n{2,})/g, '\n\n');
  }

  function cleanupBeginAndEnd (text, options) {
    return text
  ...
  function flipPunctuations (text, options) {
    var end = ['-'];
    var start = ['!', '.', '،', '…', '"'];
    var before = [];
    var after = [];

    text = fixThreeDots(text, options);
    ...
    return normalizeEllipsis(text, options);
  }

  // swap incorrect quotes pairs `»«` to `«»` and `”“` to `“”`
  function swapQuotes (text, options) {
    return text
      .replace(/(»)(.+?)(«)/g, '«$2»')
      .replace(/(”)(.+?)(“)/g, '“$2”')
    ;
