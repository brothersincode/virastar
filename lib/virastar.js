  ...
  Virastar.prototype = {
    ...
    cleanup: function cleanup (text, options) {
      ...
      if (opts.preserve_URIs) {
        ...
        // @REF: https://regexr.com/3e6m0
        text = text.replace(/(http(s)?:\/\/.)?(www\.)?[-a-zA-Z0-9@:%._\+~#=]{2,256}\.[a-z]{2,6}\b([-a-zA-Z0-9@:%_\+.~#?&//=]*)/g, function (matched) {
          uris.push(matched);
    ...
    fixEnglishNumbers: function fixEnglishNumbers (text, options) {
      return this.charBatchReplace(text, this.numbersEnglish, this.numbersPersian);
    },
