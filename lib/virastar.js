    ...
    cleanup: function cleanup (text, options) {
      ...
      if (opts.preserve_URIs) {
        ...
        text = text.replace(/\[(.*?)\]\((.*?)\)/g, function (matched, title, link) {
          ...
          return '[' + title + '](__MD_LINK__PRESERVER__)'; // no padding!
        });

        var uris = [];
        // @REF: https://stackoverflow.com/a/6927878/
        // @REF: https://daringfireball.net/2010/07/improved_regex_for_matching_urls
        text = text.replace(/\b((?:[a-z][\w-]+:(?:\/{1,3}|[a-z0-9%])|www\d{0,3}[.]|[a-z0-9.-]+[.][a-z]{2,4}\/)(?:[^\s()<>]+|\(([^\s()<>]+|(\([^\s()<>]+\)))*\))+(?:\(([^\s()<>]+|(\([^\s()<>]+\)))*\)|[^\s`!()[\]{};:'".,<>?«»“”‘’]))/g, function (matched) {
          uris.push(matched);
